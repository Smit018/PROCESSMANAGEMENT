    ...
    return (
        <div>
            {props.datasource ? props.datasource?.map((p, index) => {
                return (
                    <div className='shadow-sm m-auto w-full overflow-x-hidden p-2 h-fit bg-white cursor-pointer shadow-lx drop-shadow-sm border-2 hover:shadow-lg my-2 rounded-md'
                        onClick={() => {
                            handleClick(p);
                        }}
                        id={p.id} key={index}>
